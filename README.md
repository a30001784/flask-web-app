...
2. Public subnet is internet-facing and it  provides the environment where bastion host is serving. Engineers could only log into kubernetes worker nodes via SSH protocol on port 22 from bastion host to make sure the entire EKS cluster is  secured from the external access. The bastion host is also configured with the Kubernetes kubectl command line interface for managing the Kubernetes cluster.
